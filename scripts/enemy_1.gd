	...
	if hp<=0 and !death:
		...
		if attack:
			...
			if movingRight < 0:
				...
				$Sprite2D.position.x=-1350
				velocity.x = -SPEED
				JumpChecker.target_position=Vector2(-2500,-1500)
				WallChecker.target_position=Vector2(-1500,0)
				GroundChecker.target_position = Vector2(-1000,1000)
			else:
				state_machine.travel("run")
				$Sprite2D.scale.x = -40
				$Sprite2D.position.x=1350
